...
class ComSkew(ComMathsMixin, ComSkewDefaultsMixin, MrdsCalibMixin):
    ...
    _MIN_OPTION_PRICE = 1.0e-8  # Minimal option price
    ...
    def __init__(
        ...
        self._discount_curve = (
            discount_curve
            if discount_curve
            else DiscountCurve(mkt_date).discount_function_2
        )  # discount_function_local(mkt_date)  # DiscountCurve.discount_function(mkt_date)
        self.dcf = dcf
        ...
        self.__black_vol_inverse_tol = (
            1e-4  # default value of the black vol inverse parameter
        )

        # hashed variables
        self.__com_curve_names = None
        self.__vol_curve_names = None
        self.__factor_corr_mtx = (
            dict()
        )  # to keep track of the factor correlation matrices.
        ...
        self.__regenerate_complete_corr_mtx = (
            True  # indicator whether to regenerate the complete corr. mtx.
        )

        # some stored variables
        self.__calib_multi_thread_ind = (
            True  # indicator whether to use multi-threaded for calibration.
        )

    @classmethod
    def from_db(cls, mkt_date: datetime.date, fwd_curves: List[str]):
    ...
    def fwd_curves(self, new_fwd_curves) -> None:
        ...
        assert set(new_fwd_curves) == set(
            existing_fwd_names
        ), f"New curves {new_fwd_curves} not equal to existing ones {existing_fwd_names}"
    ...
    def fwd_curve_names(self, asset: str) -> FwdCurve:
        ...
        self.__com_curve_names = {
            fwd_curve.fwd_name: fwd_curve for fwd_curve in self.fwd_curves
        }
        return self.__com_curve_names[asset]
    ...
    def vol_curve_names(self, asset: str) -> Volatility:
        ...
        self.__vol_curve_names = {
            vol_curve.com_name: vol_curve for vol_curve in self.vol_curves
        }
    ...
    def _c_vec(self, asset: str, fwd_date: datetime.date) -> np.array:
        ...
        if asset not in self._C_vec:
            ...
            self._C_vec[asset] = {
                fwd_date: self._calibrate_skew_one_date(asset, fwd_date)
            }
        ...
        if fwd_date not in self._C_vec[asset]:
            logger.info(f"Calibrating skew params for {asset} and dates {fwd_date}")
            self._C_vec[asset][fwd_date] = self._calibrate_skew_one_date(
                asset, fwd_date
            )
    ...
    def _set_c_vec(
        self, asset: str, fwd_date: datetime.date, c_vec_value: np.array
    ) -> np.array:
    ...
    def __factor_corr_mat_default(
        self, asset_1: str, asset_2: str, same_asset_corr=0.98, diff_asset_corr=0.96
    ):
        ...
        if asset_1 == asset_2:
            return corr_hyp_sec_mat(
                same_asset_corr, range(self.nb_factors_for_asset(asset_1))
            )

        # asset_1 != asset_2
        return diff_asset_corr * np.ones(
            (self.nb_factors_for_asset(asset_1), self.nb_factors_for_asset(asset_2))
        )

    def __factor_corr_mat_lb_ub(
        self, asset_1: str, asset_2: str, lb_ub_ind="ub"
    ) -> np.ndarray:
        ...
        if asset_1 == asset_2:
            tmp_1 = np.ones(
                (self.nb_factors_for_asset(asset_1), self.nb_factors_for_asset(asset_1))
            )
        ...
        return lb_ub_fact * np.ones(
            (self.nb_factors_for_asset(asset_1), self.nb_factors_for_asset(asset_2))
        )

    def _factor_corr_mat_default(
        self, asset_1: str, asset_2: str, lb_ub_ind=None
    ) -> np.ndarray:
        ...
        mtx_to_insert = (
            self.__factor_corr_mat_default(asset_1, asset_2)
            if not lb_ub_ind
            else self.__factor_corr_mat_lb_ub(asset_1, asset_2, lb_ub_ind=lb_ub_ind)
        )
    ...
    def __factor_corr_mat_multiple(self, assets: List[str]) -> np.ndarray:
        ...
        for asset_1 in assets:
            for asset_2 in assets:
                ...
                total_corr_mat[start_ind_1:end_ind_1, start_ind_2:end_ind_2] = (
                    self._factor_corr_mat(asset_1, asset_2)
                )

        return total_corr_mat

    def _market_corr(
        self, asset_1: str, asset_2: str, mkt_corr_mtx=None
        ...
        if mkt_corr_mtx:
            logger.info(
                f"Market correlation between assets {asset_1} and {asset_2} overwritten with {mkt_corr_mtx}"
            )

        mtx_to_insert = (
            mkt_corr_mtx
            if mkt_corr_mtx
            else self.__default_factor_corr_mat(asset_1, asset_2)
        )
    ...
    def __black_corr_within_curve(
        ...
        opt_mat = self.__option_tenor_for_fwd_tenor(
            asset, fwd_date_1 if fwd_date_1 <= fwd_date_2 else fwd_date_2
        )  # opt_mat until the smallest one
        num_fwd_opt_date_1 = self.__numerical_dist_to_mktdate(
            fwd_date_1
        ) - self.__numerical_dist_to_mktdate(opt_mat)
        num_fwd_opt_date_2 = self.__numerical_dist_to_mktdate(
            fwd_date_2
        ) - self.__numerical_dist_to_mktdate(opt_mat)
        ...
        a = np.array(
            [
                corr[factor_nb_1, factor_nb_1]
                * sv[factor_nb_1]
                * sv[factor_nb_2]
                * np.product(self._beta_T[asset])
                * (
                    np.exp(
                        -kv[factor_nb_1] * num_fwd_opt_date_1
                        - kv[factor_nb_2] * num_fwd_opt_date_2
                    )
                    - np.exp(
                        -kv[factor_nb_1] * num_fwd_date_1
                        - kv[factor_nb_2] * num_fwd_date_2
                    )
                )
        ...
        return (
            (np.exp(np.sum(a)) - 1.0)
            / self.black_vol(asset, fwd_date_1, kv, sv, corr)
            / self.black_vol(asset, fwd_date_2, kv, sv, corr)
        )

    def __black_corr_intra_curves(
        self,
        ...
        opt_mat = (
            t_1 if t_1 <= t_2 else t_2
        )  # opt_mat until the smallest of the two tenors
        kv1 = self._kappa_vec(curve_1)
        kv2 = self._kappa_vec(curve_2)

        bv1 = np.sqrt(
            self.__V_current(curve_1, tenor_1, opt_mat)
        )  # square of integrated variance
        bv2 = np.sqrt(self.__V_current(curve_2, tenor_2, opt_mat))

        return sum(
            [
                model_corr_mtx[factor_nb_1, factor_nb_2]
                * self._sigma_vec(curve_1)[factor_nb_1]
                * self._sigma_vec(curve_2)[factor_nb_2]
                * self._beta_T(curve_1, tenor_1)
                * self._beta_T(curve_2, tenor_2)
                * np.exp(
                    -kv1[factor_nb_1] * self.__difference_to_market_date(tenor_1)
                ...
                * (np.exp((kv1[factor_nb_1] + kv2[factor_nb_2]) * opt_mat) - 1)
                ...
                for factor_nb_2 in range(self.nb_factors_for_asset(curve_2))
            ]
        ) / (bv1 * bv2)

    def __black_corr_intra_curves_factors(
        self,
        model_corr_mtx,
        curve_1: str,
        curve_2: str,
        ...
        bv1 = np.sqrt(self.__V_one_factor(curve_1, factor_nb_1, tenor_1, 0.0, opt_mat))
        bv2 = np.sqrt(self.__V_one_factor(curve_2, factor_nb_2, tenor_2, 0.0, opt_mat))
        ...
        return (
            ...
            * (np.exp((kv1[factor_nb_1] + kv2[factor_nb_2]) * opt_mat) - 1.0)
            / (kv1[factor_nb_1] + kv2[factor_nb_2])
            / (bv1 * bv2)
        )

    # TODO: CHECK IF THIS METHOD IS EVEN NEEDED.
    def __black_corr_intra_curves_calib(
        self, curve_1: str, curve_2: str, solver="scipy_cobyla"
    ):
        ...
        black_corr_intra_curve_vector = (
            lambda model_corr_mtx, curve_1, curve_2, corr_len: np.array(
                [
                    self.__black_corr_intra_curves(
                        model_corr_mtx, curve_1, curve_2, tenor, tenor
                    )
        ...
        black_corr_intra_curve_vector_optim = (
            lambda model_corr_mtx, curve_1, curve_2, corr_len: scipy.linalg.norm(
                black_corr_intra_curve_vector(
                    model_corr_mtx, curve_1, curve_2, corr_len
                )
        ...
        optim_pr = NSP(
            lambda corr_mtx_ravel: black_corr_intra_curve_vector_optim(
        ...
        correlation_matrix = np.array(optim_pr.xf).reshape(
            (curve_1_nb_fact, curve_2_nb_fact)
        )
    ...
    def nb_factors_for_asset(self, asset: str) -> int:
        ...
        if asset not in fwd_curve_names:
            raise ComSkewError(
                "Requested curve {0} not in list of curves: {1}".format(
                    asset, fwd_curve_names
                )
            )
    ...
    def simulation_times(
        self, sim_times: [np.ndarray, List[datetime.date], List[str], List[float]]
    ):
        ...
        if isinstance(sim_times, np.ndarray):
            return sim_times, [
                self.mkt_date + datetime.timedelta(int(np.round(stf * self.dcf)))
                for stf in sim_times
            ]

        if isinstance(sim_times, list) and isinstance(sim_times[0], datetime.datetime):
            sim_times_normalized = np.array(
                [(st - self.mkt_date).days / 365.0 for st in sim_times]
            )
            return sim_times_normalized, sim_times_normalized

        if isinstance(sim_times, list) and isinstance(sim_times[0], float):
            sim_times_normalized = np.array(sim_times)
            return sim_times_normalized, [
                self.mkt_date + datetime.timedelta(int(np.round(stf * 365.0)))
                for stf in sim_times_normalized
            ]
    ...
    def __V_current(
        self, asset: str, fwd_contract_date: datetime.date, fwd_date: datetime.date
    ):
    ...
    def __option_tenor_for_fwd_tenor(
        self, asset: str, fwd_tenor: datetime.date
    ) -> datetime.date:
    ...
    def _kappa_sigma_rho(self, asset: str) -> np.ndarray:
        ...
        fcm_init = self._factor_corr_mat_default(
            asset, asset
        )  # initial value of the factor correlation.
        fcm_lb = self._factor_corr_mat_default(
            asset, asset, lb_ub_ind="lb"
        )  # lower bound of the factor corr. mtx.
        fcm_ub = self._factor_corr_mat_default(
            asset, asset, lb_ub_ind="ub"
    ...
    def _kappa_vec(self, asset: str) -> np.ndarray:
        ...
        self._kappa_vec_val[asset] = self._kappa_sigma_rho(asset)[
            0 : self.nb_factors_for_asset(asset)
        ]
        return self._kappa_vec_val[asset]
    ...
    def _factor_corr_mat_single(self, asset: str) -> np.ndarray:
        ...
        rho_vec = self._construct_corr_asset(
            asset,
            self._kappa_sigma_rho(asset)[(2 * self.nb_factors_for_asset(asset)) :],
        )
        # transforming the rho_vec into the rho matrix
        n_dim = rho_vec.ndim
        if n_dim == 1:  # construct a 2-by-2
            rho_value = rho_vec[0]  # only 1 element
            self._rho_vec_val[asset] = np.array([[1.0, rho_value], [rho_value, 1.0]])
    ...
    def __deltas_to_strikes(
        self, asset: str, tenor_date: datetime.date, delta_vec_list: np.array
    ) -> np.array:
        ...
        integrated_vol = self.vol_curve_names(asset).atm_vol(tenor_date) * np.sqrt(
            self.__difference_to_market_date(
                self.__option_tenor_for_fwd_tenor(asset, tenor_date)
            )
        )
    ...
    def _skew_params(
        self, asset: str, C_vec: np.array, fwd_date: datetime.date
    ) -> tuple:
        ...
        v = self.black_vol_current(asset, fwd_date) * np.sqrt(
            self.__difference_to_market_date(
    ...
    def __model_vol_surface(
        ...
        C_vec,
        fwd_date: datetime.date,
        deltas: Union[np.array, None] = None,
    ) -> List[float]:
        ...
        option_tenor = self.__option_tenor_for_fwd_tenor(
            asset, fwd_date
        )  # option tenor corresponding to fwd_date
        ttm_numerical = self.__difference_to_market_date(option_tenor)
        option_prices = [
            self._polynomial_european(asset, C_vec, fwd_date, strike, cp, ttm_numerical)
    ...
    def _calibrate_skew_one_date(
        self, asset: str, fwd_date: datetime.date, deltas: List[float] = None
    ) -> np.array:
        ...
        c_vec_sol = minimize(
            lambda C_vec: scipy.linalg.norm(
                np.array(self.__model_vol_surface(asset, C_vec, fwd_date))
                - implied_vols
            ),
            initial_guess,
        )
    ...
    def _complete_corr_mtx(self, nb_steps=300) -> np.ndarray:
        ...
        total_nb_factors = sum(
            [
                self.nb_factors_for_asset(fwd_curve.fwd_name)
        ...
        for asset_1 in self.fwd_curves:
            for asset_2 in self.fwd_curves:
                ...
                self.__complete_corr_mtx[
                    self.__factor_positions(asset_1.fwd_name),
                    self.__factor_positions(asset_2.fwd_name),
                ] = self._  # self.__factor_corr_mtx(asset_1, asset_2)
        ...
        while not (np.linalg.eig(self.__complete_corr_mtx)[0] > 0.0).all():
            d1, v1 = np.linalg.eig(self.__complete_corr_mtx)
            d1p = np.diag(np.maximum(d1, 1.0e-16))
    ...
    def _var_covar_mtx(
        ...
        return self._V_cross_factor(
            asset, factor_1, factor_2, fwd_tenor, fwd_tenor, t_start, t_end
        )
    ...
    def simulate_curves(
        ...
        for com_curve in assets:
            ...
            simulated_curves[com_curve] = np.empty(
                (len(simulation_times), len(tenor_list), nb_simulations)
            )
            fwd_c_col[com_curve] = com_fwd_curve.fwd_value(tenor_list)
            simulated_curves[com_curve][0, :, :] = np.array(
                fwd_c_col[com_curve]
            ).reshape((len(tenor_list), 1))
        ...
        sim_times_numeric = [
            self.__difference_to_market_date(t_datetime)
            for t_datetime in simulation_times
        ]
        for sim_time_idx, sim_time_value in enumerate(
            sim_times_numeric
        ):  # simulation index, simulation time (in numeric terms)
            ...
            if np.all(np.linalg.eigvals(self.__factor_corr_mat_multiple(assets)) > 0):
                ...
                factor_corr_mat = near_corr_simple_iter(
                    self.__factor_corr_mat_multiple(assets)
                )  # try to make it.

            simulated_rn = self.__class__._simulate_std_normal(
                total_nb_factors, factor_corr_mat, nb_simulations
            )

            # sims_Z_unit shape = ((nb_factors_per_asset, e.g. 2) * nb_assets) X nb_simulations, e.g. 4 X 1000
            sims_Z_unit = np.dot(
                np.linalg.inv(np.linalg.cholesky(factor_corr_mat)),
                simulated_rn.transpose(),
            )

            for asset_idx, asset in enumerate(assets):  # asset like 'WTI'...
                if (
                    self.multi_thread_calib
                ):  # calibration in parallel, otherwise on the fly below in self._c_vec
                    self._c_vec_calibrate(
                        asset, tenor_list
                    )  # a little redundant, but anyways

                for tenor_idx, tenor in enumerate(
                    tenor_list
                ):  # tenor is a datetime.date format
                    # prepare cov mtx
                    nb_factors_asset = self.nb_factors_for_asset(asset)
                    cov_chol = np.linalg.cholesky(
                        np.array(
                            [
                                [
                                    self._var_covar_mtx(
                                        ...
                                        (
                                            0.0
                                            if sim_time_idx == 0
                                            else sim_times_numeric[sim_time_idx - 1]
                                        ),
                                        sim_time_value,
                                    )
                    ...
                    delta_X = np.sum(
                        np.dot(
                            cov_chol, sims_Z_unit[2 * asset_idx : 2 * asset_idx + 2, :]
                        ),
                        axis=0,
                    )
                    ...
                    skew_fom(
                        self.fwd_curve_names(asset).fwd_value(tenor),
    ...
    def simulate_1nb(
        ...
        simulation_times: List[datetime.date],
        set_seed=None,
    ) -> Dict[datetime.date, Dict[str, np.ndarray]]:
        ...
        assets_to_tenors = (
            {}
        )  # mapping where keys are assets and values are lists of tenors for that asset, e.g {'WTI': [date_1, date_2], ...}
        for asset in assets:
        ...
        simulated_curves = self.simulate_curves(
            assets,
            nb_simulations,
            simulation_times,
        ...
        assets_to_rows_in_matrix = (
            {}
        )  # mapping of assets to rows in the simulated matrix, e.g. {'WTI': [1,2,5], ... }
        for asset in assets:
            tenors_to_process = assets_to_tenors[asset]
            assets_to_rows_in_matrix[asset] = []
            for tenor in tenors_to_process:
                assets_to_rows_in_matrix[asset].append(
                    tenors_from_all_curves.index(tenor)
                )
        ...
        return {
            sim_date: {
                asset: simulated_curves[asset][
                    sim_idx, assets_to_rows_in_matrix[asset], :
                ]
                for asset in assets
            }
...
class ComSkewChecks(ComSkew):
    ...
    def check_black_vol_calib(self, asset: str, reporting_diff=1e-2) -> None:
        ...
        for fwd_curve in self.fwd_curves:
            ...
            diff = scipy.linalg.norm(
                model_atm_vols - vol_curve.implied_vol(fwd_date, K, ttm)
            )  # TODO: FIX THIS TILL THE END

            if diff > reporting_diff:
                logger.info(
                    "Calibration of ATM vols for asset {0} is LARGER than prescribed. Market - calibrated diff: {1}.".format(
                        asset, diff
                    )
                )

        logger.debug(
            "Calibration of ATM vols for asset nb. {0} succeeded. Diff = {1}".format(
                asset, str(diff)
            )
        )
