...
class tolling_model_lattice_gas():
    ...
    def transition_mtx_ln(self, delta_t=1. / 365.):
        """
    ...
    def transition_mtx_ln_blocks_sg_fast_helper(self, v, p_dash, op, F_P, F_OP, sigma_P, sigma_OP, rho, t, delta_t=1./365.,
        ...
        z_op = ne.evaluate("(log(op/F_OP) + 0.5 * sigma_OP**2 * t) / (sigma_OP * sqrt(t) )")
        # p is vector
        # z_p = (np.log(p/F_P) + 0.5 * sigma_P**2 * t) / (sigma_P * np.sqrt(t) )
        # v = (z_p - rho * z_op) / sqrt(1. - rho**2)

        d1 = ne.evaluate("(log(p_dash/F_P) + 0.5 * sigma_P**2 * (t+delta_t) - sqrt(1. - rho**2) * sigma_P * sqrt(t) * v \
    ...
    def transit_val(self, P_m, H_m, G_m):
        ...
        if self.fuel_ind:
            ...
            for F_1_ind in range(power_lattice_size):
                for F_2_ind in range(gas_lattice_size):
    ...
    def step(self, Ut_curr, Dt_curr, block_nb, start_nb=10):
        ...
        """
        profit_cap_indep = (self.lattice_seq[block_nb] - self.blocks.K) * self.hours_seq[block_nb]
        profit_max = profit_cap_indep * self.maxCap
        profit_min = profit_cap_indep * self.minCap
