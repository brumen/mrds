...
from mrds.data.discount_rates import (
    discount_curve_dates,
    discount_curve_vals,
    discount_curve_ois_rates,
)
...
vol_hash = {
    "WTI": (
        "JWSS7",
        curves_vols.wti_vol_curve_dates,
        curves_vols.wti_vol_curve_vals,
        curves_vols.wti_vol_curve_dates_vals,
    ),
    "BRENT": (
        "JWSS7",
        curves_vols.brent_vol_dates,
        curves_vols.brent_vol_vals,
        curves_vols.brent_vol_curve_dates_vals,
    ),
    "ATSI-PEAK": (
        "JWSS7",
        curves_vols.atsi_peak_vol_dates,
        curves_vols.atsi_peak_vol_vals,
        curves_vols.atsi_peak_vol_dates_vals,
    ),
    "ATSI_2X16": (
        "JWSS7",
        curves_vols.atsi_2x16_vol_dates,
        curves_vols.atsi_2x16_vol_vals,
        dict(zip(curves_vols.atsi_2x16_vol_dates, curves_vols.atsi_2x16_vol_vals)),
    ),
    "ATSI_7X8": (
        "JWSS7",
        curves_vols.atsi_2x16_vol_dates,
        curves_vols.atsi_7x8_vol_vals,
        dict(zip(curves_vols.atsi_2x16_vol_dates, curves_vols.atsi_7x8_vol_vals)),
    ),
    "NG_MICHCON_GD-PEAK": (
        "ATM",
        curves_vols.ng_michcon_gd_peak_vol_dates,
        curves_vols.ng_michcon_gd_peak_vol_vals,
        dict(
            zip(
                curves_vols.ng_michcon_gd_peak_vol_dates,
                curves_vols.ng_michcon_gd_peak_vol_vals,
    ...
    "NG_MICHCON_CASHVOL": (
        "ATM",
        curves_vols.ng_michcon_cv_vol_dates,
        curves_vols.ng_michcon_cv_vol_vals,
    ),
    "PJMW-OFFPEAK_CV": (
        "ATM",
        curves_vols.pjm_offpeak_cv_vol_dates,
        curves_vols.pjm_offpeak_cv_vol_vals,
    ),
    "PJMW-PEAK_CV": (
        "ATM",
        curves_vols.pjm_peak_cv_vol_dates,
        curves_vols.pjm_peak_cv_vol_vals,
    ),
}
...
fwd_hash = {
    "WTI": (curves_vols.wti_curve_dates, curves_vols.wti_curve_vals),
    "BRENT": brentCurve(),
    ...
    "NG_MICHCON_GD-PEAK": (
        curves_vols.ng_michcon_gd_peak_dates,
        curves_vols.ng_michcon_gd_peak_curve_vals,
    ),
    "NG_MICHCON_CASHVOL": (
        curves_vols.ng_michcon_cv_curve_dates,
        curves_vols.ng_michcon_cv_curve_vals,
    ),
    "PJMW-OFFPEAK_CV": (
        curves_vols.pjm_offpeak_cv_curve_dates,
        curves_vols.pjm_offpeak_cv_curve_vals,
    ),
    "PJMW-PEAK_CV": (
        curves_vols.pjm_peak_cv_curve_dates,
        curves_vols.pjm_peak_cv_curve_vals,
    ),
...
def get_forward_curve(
    commodity: str, mkt_date: datetime.date
...
def get_vol_curve(com_name: str, mkt_date: datetime.date) -> Tuple[str, Dict]:
...
def get_fwd_vol_curve_numeric_tenor(
    curve_name: str, mkt_date: datetime.date, fwd_vol_ind="fwd", adj_tenors_days=None
):
    ...
    if fwd_vol_ind == "fwd":
        ...
        if adj_tenors_days is not None:
            fwd_vol_tenors_vals = [
                (ot - datetime.timedelta(days=adj_tenors_days), val)
    ...
    fwd_vol_tenors_numeric = np.array([elt.days for elt in diffs]) / 365.0
    fwd_vol_tenors_code = [
        (fwt.month, fwt.year) for fwt in fwd_vol_tenors if fwt > mkt_date
    ]
    fwd_vol_values_unexpired = np.array(
        [
            fwd_vol_vals
            for fwt, fwd_vol_vals in zip(fwd_vol_tenors, fwd_vol_values)
            if fwt > mkt_date
        ]
    )

    return (
        fwd_vol_tenors_numeric,
        fwd_vol_values_unexpired,
        fwd_vol_tenors_code,
        fwd_vol_tenors,
    )


def read_data_matched_tenors(
    ...
    fwd_tenors, fwd_curve, fwd_tenors_code, fwd_tenors_dt = (
        get_fwd_vol_curve_numeric_tenor(
            fwd_curve_name,
            mktDate,
            fwd_vol_ind="fwd",
    ...
    if len(fwd_tenors_code) > len(option_tenors_code):
        match_idx = [
            (n_fwd, n_opt)
            for n_opt, item2 in enumerate(option_tenors_code)
        ...
        match_idx = [
            (n_fwd, n_opt)
            for n_opt, item2 in enumerate(fwd_tenors_code)
    ...
    option_tenors_code_final = sorting_fct(
        option_tenors_dt_matched, option_tenors_code_matched
    )
    vol_surface_params_final = sorting_fct(
        option_tenors_dt_matched, vol_surface_params_matched
    )
    ...
    return {
        "fwd_tenors": np.array(
            remove_duplicates(fwd_tenors_dt_final, fwd_tenors_final)
        ),
        "fwd_curve": np.array(remove_duplicates(fwd_tenors_dt_final, fwd_curve_final)),
        "fwd_tenors_code": np.array(
            remove_duplicates(fwd_tenors_dt_final, fwd_tenors_code_final)
        ),
        "fwd_tenors_dt": remove_duplicates(fwd_tenors_dt_final, fwd_tenors_dt_final),
        # vol part
        "option_tenors": np.array(
            remove_duplicates(option_tenors_dt_final, option_tenors_final)
        ),
        "option_tenors_code": np.array(
            remove_duplicates(option_tenors_dt_final, option_tenors_code_final)
        ),
        "option_tenors_dt": remove_duplicates(
            option_tenors_dt_final, option_tenors_dt_final
        ),
        "vol_surface_params": np.array(
            remove_duplicates(option_tenors_dt_final, vol_surface_params_final)
        ),
    }
