...
class weather():
    def __init__(self, sim_size, hp, date_l, sp=np.array([3.3, 3.3]),
        ...
        # origin, pricing, start date
        self.date_o, self.date_p, self.date_s = date_l
    ...
    def T_m_der_d(self, t_d):
        A, B, C, omega, phi, a = self.hp
        # implements: return B + C * pycuda.cumath.cos (omega * t_d + phi) *
        # omega
    ...
    def T_sim(self, t_0, t_step, T_0):
        ...
        for n in np.arange(1, self.N_step + 1):
            T_s[:, n] = self.T_step(
                t_step * n, t_step, T_s[:, n - 1], self.Z_m[:, n - 1])
    ...
    def month_decomp(self, hdd_l):
        ...
        for e in enumerate(hdd_l_sorted[1:]):
            hdd_overlap_l[
                ...
                1] = (
                last_ind,
                e[1][1])  # first index was already handled
            last_ind = e[1][1]
            ...
            def opt_fct(sl):
                ...
                hdd1 = (
                    self.HDD_real(
                        ...
                        sl_calib,
                        HDD_date_l) -
                    HDD_price_l[nb_idx])**2
                hdd2 = (
                    self.HDDO_real(
                        HDDO_price_l[nb_idx][0],
                        nb_months,
                        sl_calib,
                        HDD_date_l) -
                    HDDO_price_l[nb_idx][1])**2
    ...
    def HDD_payoff_d(self, T_sm_d):
        ...
        # rowsum_cuda_notransfer(T_sm_tmp_d, rowsum_vec_d) # writes in
        # rowsum_vec_d
    ...
    def HDD_d(self, t_0, t_step, T_0, sp):
        ...
        T_sm_d = self.T_sim_inn_d(
            T_m_v,
            T_m_d1,
            t_step,
            sp)  # T simulated matrix
    ...
    def HDDO_real(self, K, nb_months, sp_l, HDD_date_l, cp_ind="c"):
        ...
        for n in np.arange(nb_months):
            ...
            if self.gpu_ind == False:
                ...
                # write_vec_in_mat_col ( rowsum_vec_d, hdd_sim_d, n) # impl. :
                # hdd_sim_d[n,:] = rowsum_vec_d
    ...
    def HDDO_payoff_d(self, K, T_sm_d, cp_ind="c"):
        ...
        return average_reduction(
            maximum_cuda(HDD_payoff_tmp)).get / HDD_payoff_tmp.shape[0]
    ...
    def T_sim_inn(self, T_m_v, T_m_d, t_step, sp):
        ...
        return np.cumsum(
            T_s_2, axis=0) / (1 - a * t_step)**np.arange(self.N_step - 1, -1, -1).reshape((self.N_step, 1))
