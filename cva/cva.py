...
def exposure_compute(price_mat):
    """ Compute the exposure from the simulated values.

    :param price_mat: price matrix,
    """

    me_v  = np.mean(price_mat, axis=1)  # average by rows
    mpe_v = np.mean(price_mat * (price_mat >= 0.), axis=1)
    mne_v = np.mean(price_mat * (price_mat <= 0.), axis=1)
    q95_v = np.array([scipy.stats.mstats.mquantiles(x, prob=0.95) for x in price_mat]).ravel()
    q05_v = np.array([scipy.stats.mstats.mquantiles(x, prob=0.05) for x in price_mat]).ravel()
