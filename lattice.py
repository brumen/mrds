...
class lattice_ln():
    ...
    """
    def __init__(self, F0, sigma_F, sigma_C, nb_points, nb_std_dev=2.,
                 ci_ind=False, dtype_used=np.float):
        """
    ...
    def construct_lattice(self):
        ...
        if self.ci_ind:
            Z = gpa.to_gpu(Z)
            self.lattice = self.F0 * pycuda.cumath.exp(self.sigma_C * Z)
        else:
...
class lattice_2d_ln():
    ...
    """
    def __init__(self, F0, sigma_F, sigma_C, rho,
                 nb_points, nb_std_dev=2., ci_ind=False):
        """
        ...
        self.nb_points = nb_points
        self.nb_std_dev = nb_std_dev
        self.lattice = tolling.tolling_params()
        self.ci_ind = ci_ind
        self.construct_lattice()
    ...
    def construct_lattice(self):
        ...
        if self.ci_ind:
            ...
            self.lattice.gas = self.F0[1] * pycuda.cumath.exp(self.sigma_C[1] * Z)  # gas lattice
        else:
            self.lattice.power = self.F0[0] * np.exp(self.sigma_C[0] * Z)  # power lattice
            self.lattice.gas = self.F0[1] * np.exp(self.sigma_C[1] * Z)  # gas lattice
        return self.lattice
