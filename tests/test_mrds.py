...
class TestMrds(TestCase):
    ...
    model = ComSkew.from_db(MKT_DATE, ['WTI', ])

    def test_integr_analy(self):
        """ Tests the integrate analytircally method
        """

        res = self.model._ComSkew__integr_analy([1, 2], -11, -22, 3, 4, 5, 6)

        self.assertTrue(True)

    def test_c_calibration(self):

        res_1 = self.model._c_vec('WTI', datetime.date(2015, 8, 1))
    ...
    def test_simulate_curves_cpu(self):
        ...
        sim_curves = self.model.simulate_curves( ['WTI'], nb_sims, sim_times, tenor_list = tenor_list )
