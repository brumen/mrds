...
def test_dispatch_alg(nb=2**5, cuda_ind=True):
    """
    ...
    # isp = gpa.empty(nb, dtype=bool)
    # dsh = gpa.empty(nb, dtype=bool)
...
def test_simplest_toll(cuda_ind=False):
    params = test_params.test_simplest_toll_params()
    ...
    print "CF:", res['cashflow']


def test_simplest_toll_2(cuda_ind=False, nb_sim=500):
    params = test_params.test_simplest_toll_params()
    market = test_params.test_simplest_toll_market()
    days_block, hours_block, hours_block_names, days_block_names, power_bl_names,\
        fuel_idx_name, cash_vols = tolling_cmg.tolling_market_tuple(market)
...
def run_tests_1():
    ...
    print "GPU T:", time.time() - t1
    t1 = time.time()
    test_simplest_toll_2(cuda_ind=False, nb_sim=50000)
    print "CPU T:", time.time() - t1
