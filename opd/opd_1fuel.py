...
def opd_1fuel( pp
             ...
             , params
             , startup_sp_in
             , currState     # current state params
             , currDecision  # current decision
             , hours_in_block
             , nb_paths
             , cashflow_per_path ):
    """
    ...
    :param pp: power prices
    :param fp: fuel prices
    """
    ...
    state_state = currState['state']  # bool type
    ...
    # ramping costs
    generation_smaller_maxcap = currState['generation'] < max_cap
    ramp_up_to_max_cost = generation_smaller_maxcap * (ramp_up_sp_in + ramp_up_cost / (max_cap * ramp_up_horizon))
    generation_larger_mindisp = currState['generation'] > min_disp
    ...
    is_cold_start = currState['hours_shut'] >= cold_startup
    ...
    do_startup = currDecision['can_start'] * ((currDecision['force_start'] == 2) |
                                (is_startup_profitable & (currDecision['force_start'] == 1)))
    ...
    do_shutdown = currDecision['can_shut'] & ((currDecision['force_shut'] == 2) |
                                 (is_shutdown_profitable & (currDecision['force_shut'] == 1)))
    # compute dispatch
    ...
    generation_change = curr_generation - currState['generation']
    ...
    # new unit state
    currState['hours_in_state']  = currState['hours_in_state'] * (~state_change) + hours_in_block
    currState['generation']      = curr_generation
    currState['total_starts']   += starts
    currState['hours_shut']      = not_curr_state * (currState['hours_shut'] + hours_in_block)
    currState['hours_run']       = curr_state * (currState['hours_run'] + hours_in_block)
    currState['global_starts']  += starts
    currState['state']           = curr_state
