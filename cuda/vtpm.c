...
__global__ void vpm_double(double *v, double *m, int nb_cols, int nb_rows, int to_do_rows) {
  int ind1, res_idx;
  int th_idx = threadIdx.x;
  int th_bl_idx = th_idx + blockIdx.x * blockDim.x;
...
__global__ void vtm_double(double *v, double *m, int nb_cols, int nb_rows, int to_do_rows) {
  int ind1, res_idx;
  int th_idx = threadIdx.x;
  int th_bl_idx = th_idx + blockIdx.x * blockDim.x;
  __shared__ double v_cache[31];
